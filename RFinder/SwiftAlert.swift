...
class SweetAlert: UIViewController {
    ...
    func resizeAndRelayout() {
        ...
        for button in buttons {
            //let string = button.attributedTitleForState(UIControlState())
            buttonRect.append("Yes".boundingRectWithSize(CGSize(width: width, height:0.0), options: NSStringDrawingOptions.UsesLineFragmentOrigin, attributes:[NSFontAttributeName:button.titleLabel!.font], context:nil))
        }
    ...
    func showAlert(title: String, subTitle: String?, style: AlertStyle,buttonTitle: String,buttonColor: UIColor,otherButtonTitle:
        ...
        if buttonTitle.isEmpty == false {
            ...
            button.setTitle(buttonTitle, forState: UIControlState())
        ...
        if otherButtonTitle != nil && otherButtonTitle!.isEmpty == false {
            let button: UIButton = UIButton(type: UIButtonType.Custom)
            button.setTitle(otherButtonTitle, forState: UIControlState())
...
class CancelAnimatedView: AnimatableView {
    ...
    var outlineCircle: CGPath  {
        ...
        path.addArcWithCenter(CGPoint(x: self.frame.size.width/2.0, y: self.frame.size.width/2.0), radius: self.frame.size.width/2.0, startAngle: startAngle, endAngle: endAngle, clockwise: false)
...
class SuccessAnimatedView: AnimatableView {
    ...
    var outlineCircle: CGPath {
        ...
        let startAngle: CGFloat = CGFloat((0) / 180.0 * M_PI)  //0
        let endAngle: CGFloat = CGFloat((360) / 180.0 * M_PI)   //360
        path.addArcWithCenter(CGPoint(x: self.frame.size.width/2.0, y: self.frame.size.height/2.0), radius: self.frame.size.width/2.0, startAngle: startAngle, endAngle: endAngle, clockwise: false)
        return path.CGPath
    }
